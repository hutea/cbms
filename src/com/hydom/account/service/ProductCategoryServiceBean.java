...
@Service
public class ProductCategoryServiceBean extends DAOSupport<ProductCategory> implements ProductCategoryService {
	...
	public List<ProductCategory> findProductCategory(ProductCategory parent) {
		...
		//TypedQuery<ProductCategory> query = this.em.createQuery(jpql, ProductCategory.class).setFlushMode(FlushModeType.COMMIT);
	...
	public List<ProductCategory> findChildren(ProductCategory productCategory, Integer count) {
		...
		if (productCategory != null) {
			...
			query = em.createQuery(jpql, ProductCategory.class).setFlushMode(FlushModeType.COMMIT).setParameter("treePath", "%" + ProductCategory.TREE_PATH_SEPARATOR + productCategory.getId() + ProductCategory.TREE_PATH_SEPARATOR + "%");
		} else {
			String jpql = "select productCategory from ProductCategory productCategory order by productCategory.order asc";
			query = em.createQuery(jpql, ProductCategory.class).setFlushMode(FlushModeType.COMMIT);
		}
		if (count != null) {
